...
print_usage(){
cat <<EOF
Програма створює директорію, користувачів, групу та налаштовує її на віддалений доступ через samba з правами лише для даної групи
і створює ліміт по розміру, прописує у систему (fstab, smb.conf) та перезапускає службу самба, після чого усе готово для використання.

Використання: sudo bash mklimsmbdir.sh -m <Директорія> -f <Файлова система> -s <Розмір> -g <Група>

-m директорія, яку створити та розшарити (приватна)
-f тип файлової системи (або просто напиши - ext4) (один із підтримуваних mke2fs)
-s розмір у байтах (або у гігабайтах тоді закінчення GiB)
-g назва групи, якій буде доступна директорія
-h це повідомлення
